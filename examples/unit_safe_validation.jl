...
println()
println("RMSE:        ", result.rmse)               # unit-converted automatically, carries units
println("Correlation: ", round(result.correlation, digits=3))
println("Skill score: ", round(result.skill, digits=3))
println("Bias:        ", round(result.bias, digits=3), " ", Dispatch.unit_of(model), "  (bias is in model's unit — here cm/s)")
